// src/features/workout/components/WorkoutModal/GymActiveSessionStep.tsx
...
const GymActiveSessionStep: React.FC = () => {
  ...
  const { 
    ...
    selectedGymSplit,
    projectContext,
    setGymExercises, // 🚀 ADDED: Store tracking data setter
    gymExercises // 🚀 ADDED: Get current tracking data from store
  } = useWorkoutStore();
  ...
  const [exercises, setExercises] = useState<WorkoutExercise[]>(gymExercises || []);
  ...
  useEffect(() => {
    if (exercises.length > 0) {
      console.log('🔄 Syncing gym exercises to store:', exercises.length, 'exercises');
      setGymExercises(exercises);
    }
  }, [exercises, setGymExercises]);
  ...
  const handleSaveWorkout = async () => {
    try {
      ...
      console.log('🎯 Saving gym workout with tracking data:', {
      ...
      await completeWorkoutSession({
        ...
        endTime: new Date(),
        // 🚀 trackingData is automatically captured from store state via gymExercises
        // 🚀 totalVolume is automatically calculated from trackingData in store
      });
  ...
  const getWorkoutTitle = () => {
    if (selectedGymSplit) {
      // Remove "Day" and "Session" from the title, just show the split name
      return selectedGymSplit.name.replace(' Day', '').replace(' Session', '');
    }
    return 'Gym Workout';
  };
  ...
  return (
    <ThemeView style={styles.container}>
      ...
      {/* <View style={[styles.header, { backgroundColor: theme.colors.card }]}>
        <TouchableOpacity 
          onPress={handleBack} // ✅ UPDATED: Use the new handleBack function
      ...
      <View style={[styles.progressStats, { backgroundColor: theme.colors.card }]}>
        <View style={styles.statItem}>
          <ThemeText style={[styles.statValue, { color: theme.colors.primary }]}>{completedExercises}</ThemeText>
          <ThemeText style={[styles.statLabel, { color: theme.colors.text.secondary }]}>Exercises</ThemeText>
        </View>
        <View style={styles.statItem}>
          <ThemeText style={[styles.statValue, { color: theme.colors.primary }]}>{totalVolume}</ThemeText>
          <ThemeText style={[styles.statLabel, { color: theme.colors.text.secondary }]}>Volume</ThemeText>
        </View>
        <View style={styles.statItem}>
          <ThemeText style={[styles.statValue, { color: theme.colors.primary }]}>
            {exercises.reduce((total, ex) => total + ex.sets.length, 0)}
          </ThemeText>
          <ThemeText style={[styles.statLabel, { color: theme.colors.text.secondary }]}>Sets</ThemeText>
        </View>
      </View>
              ...
              <View key={exercise.id} style={[styles.exerciseCard, { backgroundColor: theme.colors.card }]}>
                ...
                >
                  <View style={styles.exerciseInfo}>
                    ...
                    {exercise.volume > 0 && (
                      <ThemeText style={[styles.exerciseVolume, { color: theme.colors.primary }]}>
                    ...
                    {trackingMode === 'sets' ? (
                              ...
                              <View style={styles.setInputs}>
                                {setIndex === 0 && (
                                  <View style={styles.inputLabels}>
                                    ...
                                    <ThemeText style={[styles.inputLabel, { color: theme.colors.text.secondary }]}>Weight ({weightUnit})</ThemeText>
                                    <ThemeText style={[styles.inputLabel, { color: theme.colors.text.secondary }]}>Done</ThemeText>
                                  </View>
                                )}
                      ...
                      <View style={styles.trackingSection}>
                        <View style={styles.prSection}>
                          ...
                          <View style={styles.prInputRow}>
                            <View style={styles.prInputContainer}>
                              <ThemeText style={[styles.prInputLabel, { color: theme.colors.text.secondary }]}>M. Reps</ThemeText>
                              <TextInput
                            ...
                            <View style={styles.prInputContainer}>
                              <ThemeText style={[styles.prInputLabel, { color: theme.colors.text.secondary }]}>Weight ({weightUnit})</ThemeText>
                              <TextInput
                                style={[styles.prInput, { 
                                  backgroundColor: theme.colors.background, 
                                  color: theme.colors.text.primary,
                        ...
                        <View style={styles.prSection}>
                          ...
                          <View style={styles.prInputRow}>
                            <View style={styles.prInputContainer}>
                              <ThemeText style={[styles.prInputLabel, { color: theme.colors.text.secondary }]}>Reps</ThemeText>
                              <TextInput
                                style={[styles.prInput, { 
                                  backgroundColor: theme.colors.background, 
                                  color: theme.colors.text.primary,
                            ...
                            <View style={styles.prInputContainer}>
                              <ThemeText style={[styles.prInputLabel, { color: theme.colors.text.secondary }]}>M. Weight ({weightUnit})</ThemeText>
                              <TextInput
                                style={[styles.prInput, { 
                                  backgroundColor: theme.colors.background, 
                                  color: theme.colors.text.primary,
      ...
      {/* Fixed Workout Actions - Updated to match old project */}
